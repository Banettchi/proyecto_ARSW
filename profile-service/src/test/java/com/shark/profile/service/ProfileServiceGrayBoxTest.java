 ...
 *
 * Justificación:
 * - Caja Gris porque conocemos la estructura interna del ProfileService
 *   (qué repositorios llama, en qué orden, con qué argumentos), pero
 *   probamos a través de la interfaz pública del servicio.
 * - Los repositorios JPA son mockeados con Mockito para aislar la capa de datos
 *   y garantizar tests rápidos y deterministas (no necesitan DB real).
 * - Se valida idempotencia (HU-07: no crear perfil duplicado), aplicación
 *   de resultados de partida, y lógica de ranking.
 */
...
class ProfileServiceGrayBoxTest {
    ...
    class CreateInitialProfileTests {
        ...
        void createProfile_UserNotExists_SavesNewProfile() {
            ...
            assertEquals("nueva_ballena", saved.getSharkName(), "sharkName debe iniciar igual que username");
            assertEquals("#00D2FF", saved.getColorHex(), "ColorHex inicial debe ser el color azul por defecto");
        ...
        void createProfile_UserAlreadyExists_DoesNothing() {
            ...
            // Verificar que NO se llamó save (idempotencia de mensajería)
    ...
    @Nested
    @DisplayName("updateSharkColor() - Actualización (HU-08)")
    class UpdateColorTests {

        @Test
        @DisplayName("Actualización de color exitosa (HU-08/KAN-22)")
    ...
    class ApplyGameResultTests {
        ...
        void applyResult_FirstTime_UpdatesScoreAndSavesHistory() {
            ...
            assertEquals(1500L, profile.getTotalScore(), "Score total debe incrementar 1000 + 500 = 1500");
        ...
        void applyResult_ProfileNotFound_CreatesDefaultProfile() {
            ...
            // No debe lanzar excepción; debe crear perfil por defecto
            assertDoesNotThrow(() ->
                    profileService.applyGameResult(new PlayerResult(userId, 3, 200L, 20), sessionId));

            // Verificamos que se guardó el perfil (llamada doble: creación + update score)
    ...
    class RankingTests {
        ...
        void getRankPosition_CalculatesCorrectly() {
            UUID userId = UUID.randomUUID();
            SharkProfile profile = SharkProfile.builder()
                    .userId(userId).totalScore(5000L)
                    .build();
        ...
        void getRankPosition_TopPlayer_Returns1() {
            UUID userId = UUID.randomUUID();
            SharkProfile profile = SharkProfile.builder()
                    .userId(userId).totalScore(99999L)
                    .build();
